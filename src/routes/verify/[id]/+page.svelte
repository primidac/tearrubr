...
<div class="min-h-screen bg-[#08080e] text-text-primary selection:bg-indigo-500/20 pb-32 print:bg-white print:text-black print:pb-0">
	...
	<div class="fixed inset-0 pointer-events-none overflow-hidden select-none z-0 print:hidden">
		<div class="absolute top-20 right-10 w-[600px] h-[600px] rounded-full bg-[#6366f1]/10 blur-[150px]"></div>
		<div class="absolute bottom-20 left-10 w-[600px] h-[600px] rounded-full bg-[#00f2fe]/10 blur-[150px]"></div>
	</div>

	<!-- Main Content Area -->
	<main class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 sm:pt-36 relative z-10 print:pt-4 print:px-0">
		
		<!-- Sub-Header / Back & System Telemetry Bar (Hidden in Print) -->
		<div class="mb-6 flex flex-wrap items-center justify-between gap-3 text-xs print:hidden">
			<a
				href="/verify"
				class="group text-[#8e8ea0] hover:text-white transition-colors inline-flex items-center gap-2 font-display"
			>
				...
				<span>Back to Authenticity Ledger</span>
			</a>

			<div class="flex items-center gap-2.5 font-mono-tight">
				<!-- The Graph Sync Badge -->
				<div class="flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/[0.03] border border-white/[0.08] text-[11px] text-[#8e8ea0]">
				...
				<!-- Sepolia Consensus Badge -->
				<div class="hidden sm:flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/[0.03] border border-white/[0.08] text-[11px] text-[#8e8ea0]">
		...
		{#if data.product}
			...
			<!-- ========================================================================= -->
			<div in:fly={{ y: 16, duration: 350 }} class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
				...
				<div class="lg:col-span-8 space-y-6">
					...
					<div class="rounded-3xl bg-[#0c0c16]/90 border border-white/[0.08] shadow-2xl backdrop-blur-2xl overflow-hidden relative print:bg-white print:border-2 print:border-black print:text-black print:shadow-none">
						...
						<div class="relative px-6 sm:px-8 py-5 border-b {sealStatus === 'opened' ? 'bg-amber-500/10 border-amber-500/20' : 'bg-emerald-500/10 border-emerald-500/20'} print:bg-gray-100 print:border-black">
							<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
								
								<!-- Verified Status Title -->
								<div class="flex items-center gap-3">
									<div class="w-11 h-11 rounded-2xl {sealStatus === 'opened' ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30' : 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30'} flex items-center justify-center shrink-0 shadow-inner">
									...
									<div>
										<div class="flex items-center gap-2 flex-wrap">
											<span class="text-xs font-bold font-mono-tight uppercase tracking-wider {sealStatus === 'opened' ? 'text-amber-300' : 'text-emerald-400'}">
												{sealStatus === 'opened' ? 'Tamper Warning: Seal Unsealed' : 'Cryptographic Certificate of Authenticity'}
											</span>
											<span class="px-2 py-0.5 rounded-full text-[10px] font-mono-tight uppercase {data.product.blockchainTxHash ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30' : 'bg-white/10 text-[#8e8ea0]'}">
												{data.product.blockchainTxHash ? 'On-Chain Validated' : 'Ledger Record'}
											</span>
										</div>
										<p class="text-xs text-[#8e8ea0] mt-0.5 print:text-gray-600">
								...
								<div class="flex items-center gap-1.5 self-start sm:self-center">
									<span class="px-2.5 py-1 rounded-full bg-black/40 border border-white/10 text-[11px] font-mono-tight text-white flex items-center gap-1.5">
										<Sparkles size={11} class="text-cyan-400" />
										<span>TearRubr ID #{truncate(data.product.id, 6, 4)}</span>
									</span>
								</div>
						...
						<div class="p-6 sm:p-8 space-y-6">
							...
							<div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 pb-6 border-b border-white/[0.06] print:border-gray-300">
								<div class="space-y-1.5">
									...
									<h2 class="text-2xl sm:text-3xl font-extrabold text-white font-display tracking-tight print:text-black">
										{data.product.name}
									</h2>
									<div class="text-sm font-semibold text-[#8e8ea0] font-display flex items-center gap-2">
								...
								<div class="p-3.5 rounded-2xl bg-white/[0.02] border border-white/[0.08] shrink-0 space-y-1 text-right sm:text-right print:border-black">
									...
									<div class="text-[10px] text-emerald-400 font-mono-tight flex items-center justify-end gap-1">
							...
							<div class="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
								<div class="p-3.5 rounded-2xl bg-white/[0.02] border border-white/[0.06] space-y-1 print:border-gray-300">
								...
								<div class="p-3.5 rounded-2xl bg-white/[0.02] border border-white/[0.06] space-y-1 print:border-gray-300">
									...
									<div class="text-xs font-bold text-white font-mono-tight print:text-black">
										{data.product.batchNumber || 'N/A (Individual)'}
									</div>
								</div>
						...
						{#if data.product.batchNumber}
							<div class="px-6 sm:px-8 py-5 bg-indigo-500/[0.04] border-t border-white/[0.06] space-y-3 print:bg-gray-50 print:border-black">
								<div class="flex items-center justify-between">
									<div class="flex items-center gap-2">
										<Boxes size={15} class="text-indigo-400" />
										<span class="text-xs font-bold text-white font-display print:text-black">Industrial Merkle Batch Audit</span>
									</div>
									<span class="text-[11px] font-mono-tight text-indigo-300 px-2 py-0.5 rounded-full bg-indigo-500/10 border border-indigo-500/20">
								...
								{#if data.batch?.merkleRoot}
									<div class="p-3 rounded-xl bg-[#08080e] border border-white/[0.08] flex items-center justify-between gap-3 text-xs font-mono-tight print:bg-white print:border-gray-300">
										<div class="truncate">
											<span class="text-[10px] text-[#7a7a8e] uppercase block">Merkle Root Hash</span>
											<span class="text-indigo-300 truncate font-mono-tight">{truncate(data.batch.merkleRoot, 18, 16)}</span>
										</div>
										<button
						...
						<div class="px-6 sm:px-8 py-6 border-t border-white/[0.06] bg-black/20 space-y-4 print:bg-white print:border-black">
							<div class="flex items-center justify-between">
							...
							<div class="space-y-2.5">
								...
								<div class="p-3.5 rounded-2xl bg-white/[0.02] border border-white/[0.06] flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 text-xs font-mono-tight print:border-gray-300">
									<div class="space-y-0.5">
										...
										<div class="text-white font-mono-tight font-semibold break-all print:text-black">
								...
								{#if data.product.blockchainTxHash}
									<div class="p-3.5 rounded-2xl bg-white/[0.02] border border-white/[0.06] flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 text-xs font-mono-tight print:border-gray-300">
										<div class="space-y-0.5">
											...
											<div class="text-emerald-400 font-mono-tight font-semibold break-all">
								...
								<div class="p-3.5 rounded-2xl bg-white/[0.02] border border-white/[0.06] flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 text-xs font-mono-tight print:border-gray-300">
									<div class="space-y-0.5">
										<span class="text-[10px] text-[#7a7a8e] uppercase flex items-center gap-1.5">
											<Layers size={11} class="text-indigo-400" />
											<span>The Graph Decentralized Subgraph Index</span>
										</span>
										<div class="text-indigo-300 font-mono-tight text-[11px]">
											Subgraph Entity ID: <strong class="text-white print:text-black">{truncate(data.product.id, 14, 12)}</strong>
											· Status: <span class="text-emerald-400 font-bold">Synchronized</span>
										</div>
									</div>

									<div class="text-[11px] font-mono-tight text-[#8e8ea0] self-start sm:self-center">
				...
				<div class="lg:col-span-4 space-y-6">
					...
					<div class="rounded-3xl p-6 bg-[#0c0c16]/90 border {sealStatus === 'opened' ? 'border-amber-500/40 shadow-[0_10px_30px_rgba(245,158,11,0.1)]' : 'border-emerald-500/40 shadow-[0_10px_30px_rgba(16,185,129,0.1)]'} backdrop-blur-2xl space-y-4 print:border-black print:bg-white">
					...
					<div class="rounded-3xl p-6 bg-[#0c0c16]/90 border border-white/[0.08] shadow-2xl backdrop-blur-2xl space-y-4 print:border-black print:bg-white">
						<div class="flex items-center justify-between pb-3 border-b border-white/[0.06] print:border-gray-300">
							<div class="flex items-center gap-2">
								<QrCode size={16} class="text-cyan-400" />
						...
						<div class="p-4 rounded-2xl bg-black/60 border border-white/[0.08] flex items-center justify-center print:bg-white print:border-gray-300">
							{#if qrDataUrl}
								...
								/>
							{:else}
								<div class="w-44 h-44 flex items-center justify-center text-xs text-[#8e8ea0]">
									Generating seal...
								</div>
							{/if}
						</div>
						...
						<div class="pt-2 flex items-center gap-2 print:hidden">
							...
							<a
								href="/dashboard"
								class="p-2 rounded-xl bg-white/[0.05] hover:bg-white/[0.1] border border-white/[0.08] text-[#8e8ea0] hover:text-white transition-colors"
					...
					<div class="p-5 rounded-3xl bg-white/[0.02] border border-white/[0.06] flex items-start gap-3.5 print:hidden">
		...
		{:else}
			...
			<!-- ========================================================================= -->
			<div class="max-w-xl mx-auto p-10 rounded-3xl bg-[#0c0c16]/90 border border-rose-500/30 text-center space-y-5 backdrop-blur-2xl shadow-2xl">
				...
				<div class="space-y-1.5">
					<h2 class="text-2xl font-extrabold text-white font-display">Cryptographic Record Not Found</h2>
					<p class="text-xs text-[#8e8ea0] max-w-sm mx-auto leading-relaxed">
						TearRubr cannot locate an authentic cryptographic record on Ethereum Sepolia for identifier <code class="text-white font-mono-tight">{data.id}</code>. This item may be uncommitted, counterfeit, or invalid.
					</p>
				</div>
